...
public class ShortestPathTree extends AbstractRoutingAlgorithm {
    ...
    public ShortestPathTree(Graph g, Weighting weighting, boolean reverseFlow, TraversalMode traversalMode) {
        ...
        this.edgeExplorer = g.createEdgeExplorer();
    }
    ...
    public void search(int from, final Consumer<IsoLabel> consumer) {
        ...
        while (!finished()) {
            ...
            while (iter.next()) {
                ...
                double nextWeight = Utility.calcWeightWithTurnWeightWithAccess(weighting, iter, reverseFlow, currentLabel.edge) + currentLabel.weight;
                if (Double.isInfinite(nextWeight))
                    continue;

                double nextDistance = iter.getDistance() + currentLabel.distance;
                long nextTime = Utility.calcMillisWithTurnMillis(weighting, iter, reverseFlow, currentLabel.edge)/1000 + currentLabel.time;
